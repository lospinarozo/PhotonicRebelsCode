...
In this file, we calculate the reflectance of the beetles from the RGB values measured from calibrated photographs to find the XXXX of polarization.
...
read_parameters_from_file <- function(camera_cat, type) {
  param <- read.table(paste0("../Data/ParametersMatLab/StandardParams", 
                             camera_cat, type, ".txt"), header=TRUE) %>% 
  select(1:5,7) %>% 
  mutate(camera_cat = rep(camera_cat)) %>% 
  mutate(type=rep(type))
...
parameter <- 
  parameter0 %>% filter(Channel != "Gy") %>% # keep only the channels R, G, B.
  select(-5) %>% # this column is the % of fitting (ideally) above 0.99)
  gather(key = parameter, 
         value = value,
         - Channel, -type, -camera_cat) #re-arrange: All parameters in 1 column
...
RGB_raw_transform <- 
  ...
  select(-Pic_code,-ANIC,-SpeciesName, -area, -min, -max) %>% 
  # Change names
  rename("type" = Filter, # This means VIS, Left polarize, or right polarized
         "Channel" = label, # This means R, G or B
         "value" = mean) %>% # This refers to the value of each channel
  filter(ind != "pind03") # removed because values were repeated accidentally
...
RGBElytraNGrey <-
  RGB_raw_transform %>% 
  filter(tr != "Pronotum") %>% # remove data for the pronotum
  unite("PhotoID", # Create a unique code
        c(1,2,4), # which contains the individual_Filter_Channel
        remove=FALSE) # still keep all the original columns
...
RGBPronot1 <-
  RGB_raw_transform %>% 
  filter(tr != "Elytra") %>%  # remove data for the elytra
  unite("PhotoID", # Create a unique code
        c(1,2,4), # which contains the individual_Filter_Channel
        remove=FALSE) # still keep all the original columns
...
RGBElGreys <- 
  RGBElGrey %>% 
  select(PhotoID,value) # select the relevant columns of the raw grey values

EQElytra <-
  merge(RGBElGreys,LinearizedElytraSpr,
        all=FALSE) %>% 
  rename(RawGrey = value) %>% 
  mutate(Equalized = 
...
RGB_ave <- EQElytra %>% 
  select(PhotoID,Equalized) %>% 
  mutate(Reflectance = Equalized * 100) %>% 
  select(-Equalized) %>% 
  separate(PhotoID,c("ind","Type","Channel"), sep="_") %>% 
  group_by(ind, Type) %>% 
  mutate(RGB_ave = mean(Reflectance)) %>% 
  select(ind, Type ,RGB_ave) %>% 
  distinct() %>%  # leave only the one row for each individual
  filter(!grepl('ambl', ind)) %>% 
  filter(!grepl('sqzb', ind)) # remove, absent in the phylogeny
...
RGB_avePr <- EQPronot %>% 
  ...
  separate(PhotoID,c("ind","Type","Channel"), sep="_") %>% 
  group_by(ind, Type) %>% 
  mutate(RGB_aveP = mean(Reflectance)) %>% 
  select(ind, Type ,RGB_aveP) %>% 
  distinct() %>%  # leave only the one row for each individual
  filter(!grepl('ambl', ind)) %>% 
  filter(!grepl('sqzb', ind)) # remove, absent in the phylogeny
...
ElyPol <- 
  RGB_ave %>% 
  spread(Type,RGB_ave) %>% 
  mutate(Polarization =  (LCP - RCP) / (LCP + RCP))
...
ElyPolbysp <-
  ElyPol %>% 
  select(ind, Polarization) %>%  
  mutate(spp= substr(ind,1,4)) # Add column for spp

EPolbs <- 
  ElyPolbysp[2:3] %>% 
  group_by(spp) %>% 
  summarise(Pol = mean(Polarization)) # by species
...
ProPol <-
  ProPol %>%
  rename(PronotPol = Polarization)

ElyPol <-
  ElyPol %>% 
  rename(ElytrPol = Polarization) #Keep all spp for other tests
...
BeetlePol <- 
  ...
  mutate(Difference = ElytrPol-PronotPol)
...
ggplot(ToPlotPolbsp, aes(x = meanPol, y = meanVIS))+
    geom_errorbar(aes(ymin = meanVIS - sdVIS,
                    ymax = meanVIS + sdVIS), 
                col="#cecec2") + 
  geom_errorbarh(aes(xmin = meanPol - sdPol,
                     xmax = meanPol + sdPol),
                 col="#cecec2") +
...
ggplot(ToPlotPolbsp, aes(x = meanPol, y = meanNIR))+
  geom_errorbar(aes(ymin = meanNIR - sdNIR,
                    ymax = meanNIR + sdNIR), 
                col="#cecec2") + 
  geom_errorbarh(aes(xmin = meanPol - sdPol,
                     xmax = meanPol + sdPol),
...
nounderlay <- 
  RPData %>% 
  filter (phylogeny_name != "Paraschizognathus_ocularis" &   
