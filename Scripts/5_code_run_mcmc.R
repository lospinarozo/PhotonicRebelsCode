...
# This function include 2 random factors:
# 1) species ("animal", for multiple individuals per species) and 2) phylogeny ("phylogeny_name")
...
# The mothod is adapted from Stuart-Fox et al. 2021 (Ecology Letters, 24, 2207– 2218).

######################################################################################


run_mcmc <- function(trees, model_chosen, phylo_data){
  ...
  temp <- name.check(trees[[1]], species) # here, the tree could be any; just ot get started.
  tree.pruned <- trees[[1]] # same here, just random tree
  AinvTA <- inverseA(tree.pruned)$Ainv 
  phylo_data$animal <- factor(phylo_data$phylogeny_name)
