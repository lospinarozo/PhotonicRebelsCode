...
pgls_runG=function(x, model, dataset) 
...
{tryCatch(
  {
    ...
    dataset_ord=match.phylo.data(tree, dataset) ### create dataset that matches order of tips for each tree
    ...
    dataord$VIS=as.numeric(as.character(dataord$VIS))
    ...
    model.results<-summary(modG)
   
    if (nrow(model.results[["coefficients"]]) == 2) #1 predictor plus intercept
      
    {
      ...
      total_results=cbind(final.results1)
