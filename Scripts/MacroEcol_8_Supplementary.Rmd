---
title: "MacroEcol_7_Supplementary"
...
ggplot(Cons, aes( full, Ratio)) + 
  geom_point(size=3, alpha=0.3, pch=21,
             col= "black",
             fill = "#762A83") + 
  xlim(10,56) + 
  ylim(0,22)+
...
Diagnostics <- 
  ...
  gather (key = spectralband, 
          value = Reflectivity)   # prepare for ggplot
...
ggplot(oz_states) +
  ...
  geom_point(
    ...
    aes(x = lon, y = lat, color = total.refl, # Change here for TOT, NIR or VIS
        size = total.refl), # if want to change size as well, size = PC2
    alpha = 0.5
  ) +
...
## Quality control for the RGB values
...
Result <- parameterTest$value[1] *
   exp(parameterTest$value[2]*test$value) +
  parameterTest$value[3] * 
  exp (parameterTest$value[4]*test$value) 

Test2 <- data.frame(test$patch_ID,Result, 
                   "Reflectance" = Result*255)
...
`Model(x) =x/(g/(K*255))` where g is the value of the grey standard in a given picture. K is the known gray reflectance (from manufacturer)

```{r}

#Test2$Reflectance[1]/ (Test2$Reflectance[4]/(0.41*255))
Test2$Result[1]/ (Test2$Reflectance[4]/(0.41))

RGB_equ[RGB_equ$patch_ID=="opal02Elyt" &
        RGB_equ$type=="LCP" &
        RGB_equ$channel=="G"  ,]


Test2$Result[1]/ (Test2$Result[4]/(0.41))
