...
```{r}
ReflByInd <- read.csv(here::here("Data/FromCode/
                                 ConsolidatedReflectivityInd.csv"))[-1]

AbsByInd0 <- read.csv(here::here("Data/FromCode/
                                 ConsolidatedAbsorptivityInd.csv"))[-1]
...
RHandByInd <- merge(RHand0I,SppNamesI) %>% 
  dplyr::rename("RHPol" = RGB_ave )

# Combine with reflectivity

ToplotRHandRefl <- merge(ReflBySpp, RHandbyInd)
rownames(RHandRefl) <- RHandRefl$spp

# Combine with absorptivity

ToplotRHandAbs <- merge(AbsBySpp, RHandbyInd)
rownames(RHandAbs) <- RHandAbs$spp
```
...
```{r}
ToPlotRHPolRefl <-
  merge(ReflByInd[,c(1:5)], RHandByInd, by = "ind") %>%
  dplyr::select(-phylogeny_name, -Ind, -ind) %>%
...
ToPlotRHPolAbs <- merge(
  ToPlotRHPolRefl[,c("spp", "meanRHPol", "sdRHPol")], AbsByInd)
rownames(ToPlotRHPolAbs) <- ToPlotRHPolAbs$spp
...
RCPNRPlot <- ggplot(data = ToPlotRHPolRefl, aes(x = meanRHPol, y = meanRes),
             size = 2.2, alpha = 0.6) +
  geom_text(
    label=rownames(ToPlotRHPolRefl), 
    nudge_x = NudgeXVal, nudge_y = NudgeYVal, 
...
RCPVAPlot <- ggplot(data = ToPlotRHPolAbs, aes(x = meanRHPol, y = meanVISA),
             size = 2.2, alpha = 0.6) +
  geom_text(
    label=rownames(ToPlotRHPolAbs), 
    nudge_x = NudgeXVal, nudge_y = NudgeYVal, 
    col="gray", size=3
  ) +
  # error bars
  ...
  geom_point(data = ToPlotRHPolAbs, size = 2.2, pch = 21, 
             fill = "#648fff", color = "black", alpha = 0.7) +
  #geom_point(data = UnderlayAbs, size = 2.4, pch = 22, fill = "#F7E949", color = "black", alpha = 0.7) +
  theme_minimal() +
  theme(legend.position = "none") + 
...
RCPNAPlot <- ggplot(data = ToPlotRHPolAbs, aes(x = meanRHPol, y = meanResA),
             size = 2.2, alpha = 0.6) +
  geom_text(
    label=rownames(ToPlotRHPolAbs), 
    nudge_x = NudgeXVal, nudge_y = NudgeYVal, 
...
grid.arrange(RCPVRPlot, RCPNRPlot, RCPVAPlot, RCPNAPlot, nrow = 2)
```