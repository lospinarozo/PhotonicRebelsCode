...
write.csv(FinalPCs,"../Data/FromCode/PCsbySpp.csv")
...
Cons1 <- data.frame(Cons1oo, PC_Values) %>% 
  dplyr::select(-Species, -phylogeny_name.1)
...
Checking for correlations between PCs and Size

PC1 and Size:

```{r}

cor.test(Cons1$size,Cons1$PC1)

cor.test(Cons1$size,Cons1$PC2)
...
szp1 <- ggplot(Cons1, aes(x = size, y = PC1)) +
  geom_point(fill="#39BEB1",col="black",pch=21,size=3.2,alpha=0.4)+
  theme_bw()+
  geom_smooth(method = lm,
              color="darkslategray")

szp2 <- ggplot(Cons1, aes(x = size, y = PC2)) +
  geom_point(fill="#39BEB1",col="black",pch=21,size=3.2,alpha=0.4)+
  theme_bw()+
  geom_smooth(method = lm,
              color="darkslategray")
