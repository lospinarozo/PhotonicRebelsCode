...
library(here)
Beetles <- read.csv(here("Data/SupplementaryBeetleDataBase.csv"))
library(DT)
datatable(Beetles)
```
...
```{r, warning=FALSE, message=FALSE}
source(here("Scripts/MacroEcol_1_Libraries.R"))
```
...
# Import data
refl1 <- read.csv(here("Data/1_Reflectance_HRBatch.csv")) # HR batch
refl2 <- read.csv(here("Data/2_Reflectance_FirstBatch.csv")) # original
TopTrans <- read.csv(here("Data/3_Transmittance_HRBatch.csv")) # transmittance
theorySun <- read.csv(here("Data/4_SunIrradiance.csv")) # Sun irradiance
...
write.csv(Reflectance, here("Data/FromCode/Reflectance_PData.csv"))
write.csv(Transmitance, here("Data/FromCode/Transmitance_PData.csv"))
```
