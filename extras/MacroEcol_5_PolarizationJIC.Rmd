...
write.csv(RGB_ave, "../Data/FromCode/PolarzElytra_LERGBValues.csv")
...
## Correlations with other optical properties
<br>

This was an explratory analysis to evaluate the general correlations between the optical variables. However, further in the next section we repeated this step while considering the mechanisms (as it is presented in the manuscript)
...
It has been hypothesized that the chiral structures associated to polarization sensitivity are more effective reflecting light than pigments or other structures. This may be true if coupled with disorder and additional structures to produce broad band colours. However, it is also possible that simple chiral structures produce highly saturated angle-dependent colours which would mean reflecting a very narrow section of the spectrum.  

**Is a higer degree of polarization correlated with more reflectivity in the visible spectrum?**
...
modPolnirspp <- lm(RPDataAgg$R_NIR_1 ~ RPDataAgg$ElytrPol_1) # by spp
summary(modPolnirspp)
```
...
## Correlations considering mechanisms
<br>
<br>

The section chiral vs. non-chiral only considers two mechanisms, however in our manuscript we divided the beetles into three categories according to the available knowledge of their mechanisms. Thus, these two sections are just two ways to study the relevance of the mechanism
...
Based on the available knowledge about the mechanisms in beetles, we can consider three broad categories: a simple chiral mechanism, a composite chiral mechanism and a composite non-chiral (white underlay)

We can analyze correlations between reflectivity in VIS and NIR, and polarisation considering the mechanism 
<br>
...
cor.test(nospecialcasesspp$meanVIS, nospecialcasesspp$meanPol)
...
ggplot(data = ToPlotPolbsp, aes(x = meanPol, y = meanVIS),
  ...
  # composite non-chiral
  geom_point(data = WhiteUnderlay, aes(x = meanPol, y = meanVIS),
             size = 2.2, pch = 25, fill= "#785ef0", colour = "black", alpha = 0.7) +
  
  # Composite Chiral
  geom_point(data = ChiralComposite, aes(x = meanPol, y = meanVIS),
             size = 2.2, pch = 23, fill= "#fe6100", colour = "black", alpha = 0.7) +
...
These are the models excluding composite chiral and composite non-chiral
...
cor.test(nospecialcasesspp$meanNIR, nospecialcasesspp$meanPol)
...
ggplot(data = ToPlotPolbsp, aes(x = meanPol, y = meanNIR),
  ...
  # Composite chiral
  geom_point(data = ChiralComposite, aes(x = meanPol, y = meanNIR),
             size = 2.2, pch = 23, colour = "black", fill= "#fe6100",  alpha = 0.7) +
...
############# All the chunks in the section above should be removed or relocated ##################
...
write.csv(ElyPolbyindN, "../Data/FromCode/PolarzElytraByInd.csv") # by individual
write.csv(EPolbs, "../Data/FromCode/PolarzElytraBySpp.csv") # by species
